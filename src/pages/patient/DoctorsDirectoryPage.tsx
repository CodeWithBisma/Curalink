...
export const DoctorsDirectoryPage: React.FC = () => {
  ...
  const filteredDoctors = approvedDoctors.filter(d => {
    ...
    if (budgetFilter !== 'all') {
      let fee = d.video_fee;
      if (visitTypeFilter === 'home_visit') {
        fee = d.home_visit_fee || d.video_fee;
      } else if (visitTypeFilter === 'all') {
        fee = Math.min(d.video_fee, d.offers_home_visit && d.home_visit_fee ? d.home_visit_fee : d.video_fee);
      }

      if (budgetFilter === 'under_1500' && fee > 1500) return false;
      if (budgetFilter === '1500_2500' && (fee < 1500 || fee > 2500)) return false;
      if (budgetFilter === '2500_3500' && (fee < 2500 || fee > 3500)) return false;
      if (budgetFilter === 'above_3500' && fee <= 3500) return false;
    }

    // 3. 24-hour and day availability filters
    ...
    if (visitTypeFilter === 'home_visit' && !d.offers_home_visit) {
      return false;
    }
  ...
  const getBudgetLabel = (key: string) => {
    switch (key) {
      case 'under_1500': return 'Under Rs. 1,500';
      case '1500_2500': return 'Rs. 1,500 – 2,500';
      case '2500_3500': return 'Rs. 2,500 – 3,500';
      case 'above_3500': return 'Rs. 3,500+';
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div className="space-y-2">
          ...
          <p className="text-xs sm:text-sm text-gray-600">
            Filter doctors first by your city and budget to find PMC-verified general physicians and specialists near you.
          </p>
        </div>
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-4 sm:p-5 shadow-xs space-y-4">
        ...
        <div className="space-y-1.5 pt-1">
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-bold text-gray-600 flex items-center gap-1.5">
              ...
              <span>Filter by Patient Budget:</span>
            </span>
            {budgetFilter !== 'all' && (
              <span className="text-[11px] text-teal-700 font-semibold">
                Showing {getBudgetLabel(budgetFilter)}
              </span>
            )}
          </div>
          <div className="flex flex-wrap items-center gap-2">
            {[
              ...
              { id: 'under_1500', label: 'Under Rs. 1,500 (Economical)' },
              { id: '1500_2500', label: 'Rs. 1,500 – 2,500 (Standard)' },
              { id: '2500_3500', label: 'Rs. 2,500 – 3,500 (Specialist)' },
              { id: 'above_3500', label: 'Rs. 3,500+ (Consultant)' }
            ].map(b => {
              const isSelected = budgetFilter === b.id;
              return (
                <button
                  key={b.id}
                  id={`budget-filter-${b.id}`}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 pt-2 border-t border-gray-100">
          ...
          <div>
            ...
            <select
              ...
              className="w-full text-xs px-2.5 py-2 rounded-lg border border-gray-300 focus:outline-none focus:border-teal-600 bg-white font-medium"
            >
              <option value="all">Any Fee / Budget</option>
              <option value="under_1500">Under Rs. 1,500</option>
              <option value="1500_2500">Rs. 1,500 – 2,500</option>
              <option value="2500_3500">Rs. 2,500 – 3,500</option>
              <option value="above_3500">Rs. 3,500+</option>
            </select>
          </div>
          ...
          <div>
            ...
            >
              ...
              <option value="next_24h">⚡ Within 24 Hours</option>
        ...
        {(availabilityFilter !== 'all' || specialtyFilter !== 'all' || visitTypeFilter !== 'all' || cityFilter !== 'all' || budgetFilter !== 'all' || searchQuery) && (
          <div className="flex flex-wrap items-center justify-between pt-2 border-t border-gray-100 text-xs text-gray-500 gap-2">
            <div className="flex flex-wrap items-center gap-2">
              ...
              {cityFilter !== 'all' && (
                <span className="inline-flex items-center gap-1 bg-teal-100 text-teal-900 px-2 py-0.5 rounded-md text-[11px] font-bold">
                  <MapPin className="w-3 h-3 text-teal-700" />
                  City: {cityFilter}
                </span>
              )}
              {budgetFilter !== 'all' && (
                <span className="inline-flex items-center gap-1 bg-emerald-100 text-emerald-900 px-2 py-0.5 rounded-md text-[11px] font-bold">
              ...
              {visitTypeFilter !== 'all' && (
                <span className="bg-gray-100 text-gray-800 px-2 py-0.5 rounded-md text-[11px]">{visitTypeFilter === 'video' ? 'Video' : 'Home Visit'}</span>
              )}
            </div>
            <button
      ...
      {filteredDoctors.length > 0 ? (
                    ...
                    {doctor.offers_home_visit && (
                      <div className="flex items-center justify-between">
                        <span className="flex items-center gap-1.5 text-emerald-800 font-semibold text-[11px]">
                          ...
                          Home Visit ({doctor.home_visit_radius_km}km)
                        </span>
                        <span className="font-bold text-gray-900">Rs. {doctor.home_visit_fee.toLocaleString()}</span>
                      </div>
                    )}
                  </div>
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-12 text-center space-y-4 shadow-xs">
          ...
          <h3 className="text-base font-bold text-gray-900">No doctors match your criteria</h3>
          <p className="text-xs text-gray-500 max-w-md mx-auto">
            {budgetFilter !== 'all' || cityFilter !== 'all'
              ? `No doctors found ${cityFilter !== 'all' ? `in ${cityFilter}` : ''} ${budgetFilter !== 'all' ? `within ${getBudgetLabel(budgetFilter)}` : ''}. Try broadening your budget filter or choosing "All Cities".`
              : availabilityFilter === 'next_24h'
              ...
              : 'Try adjusting your specialty, city, budget, or visit type filters to view more verified practitioners in Pakistan.'}
          </p>
          <button
            onClick={resetFilters}
