          ...
          <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-xs space-y-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2 text-emerald-600">
                ...
                <h3 className="text-sm font-bold text-gray-900">Home Medical Visit</h3>
              </div>
              <span className="text-base font-extrabold text-gray-900">Rs. {doctor.home_visit_fee.toLocaleString()}</span>
            </div>
            <p className="text-xs text-gray-500 leading-relaxed">
              Bedside physical examination in {doctor.city} within a {doctor.home_visit_radius_km} km radius. Real-time GPS doctor arrival tracking.
            </p>
            <button
              onClick={() => navigate(`/booking/new?doctorId=${docId}&type=home_visit`)}
              className="w-full bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold py-2.5 rounded-lg transition-colors cursor-pointer"
            >
              Request Home Visit
            </button>
          </div>
