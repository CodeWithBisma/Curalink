...
export const PatientDashboardPage: React.FC = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6 pb-20">
      ...
      {/* TAB 2.5: DEDICATED MEDICAL NOTES VIEW */}
      {activeTab === 'notes' && (
        <MedicalNotesSection />
      )}
            ...
            <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-xs space-y-4">
              <div className="flex items-center justify-between border-b border-gray-100 pb-4">
                <div>
                  <h2 className="text-base font-bold text-gray-900 flex items-center gap-2">
                    ...
                    <span>Recent Medical & Clinical Notes</span>
                  </h2>
                  <p className="text-xs text-gray-500">Doctor remarks, home vitals, symptom tracking, and allergy warnings.</p>
                </div>
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => navigate('/dashboard/patient/notes')}
                    className="text-xs font-bold text-[#0F766E] hover:underline flex items-center gap-1 cursor-pointer"
                  >
                    <span>All Notes ({(medicalNotes || []).length})</span>
                    <ChevronRight className="w-3.5 h-3.5" />
                  </button>
                </div>
              </div>
              ...
              {(medicalNotes || []).length > 0 ? (
                  ...
                  {(medicalNotes || []).slice(0, 2).map((note) => (
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between text-[11px] text-gray-400 pt-1">
                        <span>By {note.author_name} ({note.author_role})</span>
                ...
                <div className="p-4 bg-gray-50 border border-gray-100 rounded-xl text-center">
                  <p className="text-xs text-gray-500">No medical notes created yet.</p>
                </div>
              )}
              ...
              <div className="pt-2 flex justify-end">
                <button
                  onClick={() => navigate('/dashboard/patient/notes')}
                  className="bg-teal-50 hover:bg-teal-100 text-[#0F766E] font-bold text-xs px-3.5 py-2 rounded-xl transition-colors inline-flex items-center gap-1.5 cursor-pointer"
                >
                  <Plus className="w-3.5 h-3.5" />
                  <span>Open Medical Notes Dashboard</span>
                </button>
              </div>
            </div>
          </div>
