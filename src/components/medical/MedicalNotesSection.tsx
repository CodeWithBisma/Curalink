...
import { MedicalNote, MedicalNoteCategory, MedicalNotePriority } from '../../types';
import {
  ClipboardList,
  Pin,
  PinOff,
  ...
  const handleSaveNote = (e: React.FormEvent) => {
    ...
    if (editingNote) {
      ...
      addMedicalNote({
        ...
        vitals: Object.keys(vitalsObj).length > 0 ? vitalsObj : undefined,
        leave_details: leaveObj,
        patient_cnic: leaveObj ? leaveObj.patient_cnic : verifiedCnic
      });
    }
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-lg sm:text-xl font-bold text-gray-900">Medical & Clinical Notes</h2>
              ...
              <p className="text-xs sm:text-sm text-gray-500 mt-1">
                Permanent health logs, doctor observations, home vitals, and critical allergy alerts for{' '}
                <span className="font-semibold text-gray-800">{targetPatientName}</span>.
              </p>
            </div>
          </div>
      ...
      {filteredNotes.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filteredNotes.map((note) => {
            ...
            const canModify = !readOnly && (currentUser?.uid === note.author_uid || currentUser?.role === 'patient');
