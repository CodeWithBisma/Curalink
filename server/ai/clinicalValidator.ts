...
export function validateAndNormalizeClinicalReport(raw: any, transcriptText?: string): ValidationResult {
  ...
  const patientGender = raw.patient_gender || (transcript.includes('female') || transcript.includes(' ms ') || transcript.includes(' mrs ') ? 'Female' : transcript.includes(' male ') || transcript.includes(' mr ') ? 'Male' : undefined);
